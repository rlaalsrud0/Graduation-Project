...
try:
    while True:
        ...
        if(distance < 30) :
            ...
            filename = str(datetime.date.today()) + "_in"
...
try:
    while True:
        ...
        if(distance < 30) :
            ...
            val=list(set(resultB).intersection(resultA))   
            print(val[0])
            ...
            sql = "SELECT h_seq FROM home WHERE h_num = '" + val[0] + "' order by h_odate desc limit 1"
            ...
            iroute = 'ADDRESS' + filename2
            
            if cursor.rowcount > 0 :
                sql1 = "UPDATE home SET h_idate = '" + time.strftime('%Y-%m-%d %H:%M:%S').strip() + "', h_iroute = '"+ iroute + "' WHERE h_seq = " + str(result[0][0])
                #val1 = (now, "video", str(result[0][0]))
                cursor.execute(sql1)
