...
class VListPro : AppCompatActivity() {
    private var retrofit: Retrofit? = null
    private var retrofitInterface: RetrofitInterface? = null

    // url로 파라미터 요청하기
    private val BASE_URL = "ADDRESS:1045"

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.v_list_pro)
        ...
        call?.enqueue(object : Callback<Void?> {
            override fun onResponse(
                ...
                if (response.code() == 200) {
                    // js에서 보호자 이름 가져오기
                    jsgetname()
                    Log.e(TAG, "js에 나의 ssaid 전송 성공")
                } else if (response.code() == 400) {
                    Log.e(TAG, "js에 나의 ssaid 전송 실패")
                }
            }
    ...
    private fun jsgetname() { // 이름을 js에서 가져오는 코드
        Log.e(TAG, "jsgetname()함수 실행 성공 ")

        val map = HashMap<String?, String?>()

        var call = retrofitInterface!!.getPName(map)
    ...
    private fun parseJson(json: String) {
        ...
        lateinit var adapter: PListAdapter

        recyclerView = findViewById(R.id.recyclerView)

        recyclerView.layoutManager = LinearLayoutManager(this@VListPro)
        ...
        try {
            ...
            for (i in 0 until jsonObject.length()) {
                ...
                pNameList.add(u_name)

                Log.e(TAG, "add 성공")

                adapter = PListAdapter(pNameList, this@VListPro)
